...
norms = {'ap': 2, 'lo': 1.25, 'mb': 1.5, 'sg': 2, 'ss': 2}
labels = BASKET_LABEL + 'abcdefghijklmnopqrstuvwxyz'

for map_id in maps:
    coords = genfromtxt(f"gusher graphs/{map_id}/gushers.csv", delimiter=', ')
    adjacency_matrix = around(0.25*distance_matrix(coords, coords, p=norms[map_id]), decimals=2)
    header_str = maps[map_id][0] + '\n' + \
    ...
    savetxt(f"gusher graphs/{map_id}/distances.txt", adjacency_matrix,
            fmt='% 6.2f', delimiter=',', header=header_str, comments=COMMENT_CHAR+' ')
